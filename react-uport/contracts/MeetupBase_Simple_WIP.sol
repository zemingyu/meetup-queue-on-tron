...
contract MeetupBase is MeetupAccessControl {
    ...
    struct Meetup {
        ...
        // the top N users on this list will be automatically moved to the registration list
        // The ranking will be changed to a bidding process process later on
        // For now, just rank users based on when they go on the waitin glist
        address[] waitingList;

        // Cursor of the waiting list, so anything before the cursor is ignored
        // See https://ethereum.stackexchange.com/questions/13973/how-to-return-a-exact-value-pushed-before-in-my-contract
        uint waitingListCursor;

        // Mapping to track if a user on the waiting list has decided to deregister
...
//   }
        ...
        // returns (bool)
    {
        ...
        bool rejoinWaitingList = false;

        for (i = 0; i < _meetup.waitingList.length; i++) {
            if (_meetup.waitingList[i] == msg.sender && isWaiting[msg.sender] == true) {
                revert();
            }

            if (_meetup.waitingList[i] == msg.sender && isWaiting[msg.sender] == false) {
                rejoinWaitingList = true;                    
            }
        }

        // If there is any capacity remaining
        if (_meetup.remainingCapacity > 0) {
          ...
          _meetup.remainingCapacity = sub(_meetup.remainingCapacity, 1);
            ...
            if (!rejoinWaitingList) {
                // Join the waiting list for the fist time
        ...
        // returns (bool)
    {
        ...
        // Can't deregister for a meetup that has already started.
        require(now < _meetup.startTime);

        uint occupiedCapacity = sub(_meetup.maxCapacity, _meetup.remainingCapacity);

        // Need to be in the list to deregister
        uint registeredStatus = 0;

        uint registrationListIndex;
        uint waitingListIndex;

        for (uint i = 0; i < occupiedCapacity; i++) {
            if (_meetup.registrationList[i] == msg.sender) {
                registeredStatus = 1;
                registrationListIndex = i;
            }
        }
        ...
        require(registeredStatus > 0);

        // If is in the registration list
        if (registeredStatus == 1) {
            // Remove user from the registration list
            // Replace with the last address in the registration list
            _meetup.registrationList[registrationListIndex] = _meetup.registrationList[occupiedCapacity];
            _meetup.registrationList[occupiedCapacity] = 0x0;

            // If there are people on the waiting list, then move one into the registration list
            // Only move if the user is still waiting
            if (_meetup.waitingList[waitingListCursor] != 0x0) {

                for (i = _meetup.waitingListCursor; i < _meetup.waitingList.length; i++) {
                    if (_meetup.isWaiting[ _meetup.waitingList[i] ] == true) {
                        _meetup.registrationList[occupiedCapacity] = _meetup.waitingList[i];                        

                        // Get to the end of the waiting list
                        if (i == waitingList.length) {


                        } else {
                            _meetup.waitingListCursor = i + 1;
